...
close_check: {"cmd": "grep -qvx 0 /sys/fs/cgroup/user.slice/user-1000.slice/user@1000.service/app.slice/memory.low", "expect": "exit_zero", "porque": "lee la MAQUINA y no el repo: un drop-in copiado que systemd no aplico deja los ficheros en su sitio y memory.low en 0, asi que un criterio sobre ficheros se cerraria en falso. Mira app.slice, que es el eslabon por el que la proteccion ATRAVIESA hasta los scopes de ventana -- el sujeto de esta ficha. Estuvo un rato apuntando a user-1000.slice porque pii-scan leia esta ruta como una direccion de correo; se arreglo declarando el nombre de la unit en tasks/pii_allow.txt en vez de escribir una ruta que no existe."}
