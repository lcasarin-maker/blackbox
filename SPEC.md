...
| `tools/presupuesto_memoria.py` | comprueba que los techos de memoria **compongan**: `MemTotal` contra la suma de los techos de cada slice más la reserva de memoria unificada de GPU, que ningún cgroup ve (medido aquí: 7 GiB de CUDA → 15 MiB contabilizados). La reserva sale del máximo observado en 18 733 muestras (85.4 GiB), no de lo que el vLLM declara — su `--gpu-memory-utilization 0.3` son 36.3 GiB y los picos diarios lo superan todos los días | `tests/test_presupuesto_memoria.py`, 8 casos: falla con el reparto real (168.3 sobre 121.1) y pasa cuando los techos caben, porque un presupuesto que siempre cuadra no es un presupuesto |
...
| `bin/bb` es el 100% del código ejecutable y ningún gate mide su cobertura | **Ocurrió** | `coverage.py` no instrumenta bash, así que `coverage-target` pasaba sin mirarlo. `tools/cobertura_bash.sh` lo mide (**27.5%** el 2026-09-25, desde 17.7% el 2026-09-24, al probar `bb status`, `cpu_top`, `swap`, `swap.top`, `latencia_x`, la cadena de `memory.low` y los tres chequeos que pasaron a leer el dato; el piso queda en 27.0 porque la cifra VARÍA entre corridas — parte de la suite lee el estado real de la máquina — y un trinquete puesto en el máximo de una medida ruidosa se vuelve intermitente y acaba ignorándose) y `bb-cobertura-piso` impide que baje. El número sigue siendo bajo y es el hallazgo, no la solución |
