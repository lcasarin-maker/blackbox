...
def test_pidio_NOMBRA_a_quien_pide_memoria(datos):
    ...
    try:
        time.sleep(3)  # blocking-sleep: se espera a que el hijo reserve -- DEBT-ACCEPTED-SLEEP-TESTS-BB
...
def test_control_negativo_un_proceso_que_NO_pide_no_sale_nombrado(datos):
    ...
    try:
        ...
        correr(["sample"], datos)
        time.sleep(2)  # blocking-sleep: separa las dos muestras -- DEBT-ACCEPTED-SLEEP-TESTS-BB
        correr(["sample"], datos)
...
def test_residuo_es_un_numero_y_no_se_mueve_solo(datos):
    ...
    correr(["sample"], datos)
    time.sleep(1)  # blocking-sleep: dos muestras distintas -- DEBT-ACCEPTED-SLEEP-TESTS-BB
    correr(["sample"], datos)
